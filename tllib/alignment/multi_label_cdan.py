...
class ConditionalDomainAdversarialLoss(nn.Module):
    ...
    def __init__(self, domain_discriminator: nn.Module, entropy_conditioning: Optional[bool] = False,
                 ...
                 reduction: Optional[str] = 'mean', sigmoid=True, type='mc'):
    ...
    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor, multilabel2multiclass=False) -> torch.Tensor:
        # multiple class; single label; multiple label
        f = torch.cat((f_s, f_t), dim=0)
        g = torch.cat((g_s, g_t), dim=0)
        if self.type == 'mc':
            g = F.softmax(g, dim=1).detach() # (2b, num_classes)
        elif self.type == 'sl':
            assert g.size(1) == 1
            g_p = torch.sigmoid(g).detach()
            g_n = 1. - g_p
            g = torch.cat((g_p, g_n), dim=1) # (2b, 2)
        elif self.type == 'ml':
            g = torch.sigmoid(g).detach() # (2b, num_classes)
            if multilabel2multiclass:
                g = self.transform_label(g) # (2b, pow(2,num_classes))
