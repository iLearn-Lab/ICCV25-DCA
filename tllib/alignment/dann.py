    ...
    def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
        ...
        if self.sigmoid:
            ...
            self.domain_discriminator_accuracy = 0.5 * (
                        binary_accuracy_original(d_s, d_label_s) + binary_accuracy_original(d_t, d_label_t))
...
class ImageClassifierAdam(ClassifierBase):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256, **kwargs):
        bottleneck = nn.Sequential(
            # nn.AdaptiveAvgPool2d(output_size=(1, 1)),
            # nn.Flatten(),
        ...
        super(ImageClassifierAdam, self).__init__(backbone, num_classes, bottleneck, bottleneck_dim, **kwargs)

# class ImageClassifier2(ClassifierBase):
#     def __init__(self, backbone: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256, **kwargs):
#         bottleneck = nn.Sequential(
#             # nn.AdaptiveAvgPool2d(output_size=(1, 1)),
#             # nn.Flatten(),
...
class ImageClassifier2(ClassifierBase):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256, **kwargs):
        ...
        super(ImageClassifier2, self).__init__(backbone, num_classes, bottleneck, bottleneck_dim, **kwargs)
