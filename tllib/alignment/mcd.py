...
def multi_label_entropy(predictions: torch.Tensor) -> torch.Tensor:

    return -torch.mean(torch.log(torch.mean(torch.sigmoid(predictions)-torch.tensor(0.5).cuda(), 0) + 1e-6))
