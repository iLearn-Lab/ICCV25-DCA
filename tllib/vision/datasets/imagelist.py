...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, multi_labels: [bool] = False,
                 transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
        super().__init__(root, transform=transform, target_transform=target_transform)
        self.classes = classes
        if multi_labels:
            self.samples = self.multi_labels_parse_data_file(data_list_file)
        else:
    ...
    def multi_labels_parse_data_file(self, file_name: str) -> List[Tuple[str, List[int]]]:
        """Parse file to data list
