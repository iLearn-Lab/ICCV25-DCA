...
def collect_d_output(data_loader: DataLoader, model: ImageClassifier, domain_adv: DomainAdversarialLoss,
                    device: torch.device, max_num_features=None) -> torch.Tensor:
    """
    Fetch data from `data_loader`, and then use `feature_extractor` to collect features
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm.tqdm(data_loader)):
            ...
            inputs = data[0].to(device)
            y,f = model(inputs)
            d = domain_adv.domain_discriminator(f)
            all_output.extend(d.cpu().numpy())
    return all_output
