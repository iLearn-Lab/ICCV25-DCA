...
class ClassAwareSampler (Sampler):
    ...
    def __init__(self, data_target, num_samples_cls=1,):
        num_classes = len(np.unique(data_target))
        self.class_iter = RandomCycleIter(range(num_classes))
        cls_data_list = [list() for _ in range(num_classes)]
        for i, label in enumerate(data_target):
...
def get_samplers(data_targets):
    # data_targets: list
    data_targets_array = np.array(data_targets)
    samplers = []
    for i in range(data_targets_array.shape[1]):
        samplers.append(ClassAwareSampler(data_targets_array[:, i]))
    return samplers


def get_sampler(data_targets):
    # data_targets: list
    data_targets_array = np.array(data_targets)
    sampler = ClassAwareSampler(data_targets_array)
    return sampler
