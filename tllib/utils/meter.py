...
class AverageVectorMeter(object):
    ...
    Examples::
        ...
        >>> losses.update(loss_value, batch_size)
    """
    def __init__(self, name: str, device, fmt: Optional[str] = ':f', dim=36):
        self.name = name
        self.fmt = fmt
        self.dim = dim
        self.device = device
    ...
    def reset(self):
        self.val = torch.zeros(self.dim).to(self.device)
        self.avg = torch.zeros(self.dim).to(self.device)
        self.sum = torch.zeros(self.dim).to(self.device)
        self.count = 0
